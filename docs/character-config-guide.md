...
## Troubleshooting: Init Flow

Common failures when using `spriteforge init`:

| Symptom | Likely cause | Fix |
|---------|-------------|-----|
| `--character-name is required with --non-interactive` | `--non-interactive` used without `--character-name` | Add `--character-name "..."` |
| `Invalid value: --action must be in format: name|movement description|frames|timing_ms` | `--action` value doesn't have exactly four pipe-separated fields | Use `"name|movement description|frames|timing_ms"` format, e.g. `"idle|breathing in place|4|120"` |
...
| `estimate` reports 0 calls for a row | `frames: 0` in an animation | Re-run `init` with corrected `--action` entries |

See [`docs/maintainer-validation-checklist.md`](./maintainer-validation-checklist.md) for the full pre-release smoke-check procedure.
